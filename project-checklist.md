...
## 2026-06-10
- [ ] Спроектировать mobile-first `Grace restart` экран для пропуска 24–72 часа: evidence snippet + grace day + микро-действие ≤5 минут + calm restart option — зачем: цепочка книга -> действие -> привычка должна восстанавливать пользователя после паузы; метрика/риск: `grace_day_used`, `habit_restart_after_gap_completed`, return-after-gap conversion
